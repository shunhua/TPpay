...
class String {
    ...
    public function formatMoney($money){
        $money=round($money,2);
        $floatPoint=explode('.',(string)$money);
        if($floatPoint[1]=='00') $money=round($money,0);
        else if(preg_match('/[0-9]0/',$floatPoint[1])) $money=round($money,1);
        return $money;
    }
    /**
     * 对数组中的金额进行格式化输出
     * @param array $data 数组
     * @param array $need 数组中需要转换的键值
     * @return float|int
     * @author demo
     */
    public function formatMoneyByArray($data,$need){
        foreach($need as $iNeed){
            $data[$iNeed]=$this->formatMoney($data[$iNeed]);
        }
        return $data;
    }
    /**
