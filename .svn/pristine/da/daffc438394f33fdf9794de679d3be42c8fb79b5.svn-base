...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if($inputObj->GetTrade_type() == "MWEB"){
			$inputObj->SetSceneInfo('{"h5_info": {"type":"Wap","wap_url": "http://'.$_SERVER["HTTP_HOST"].'","wap_name": "轻支付"}}' );//场景信息
		}

		//异步通知url未设置，则使用配置文件中的url
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url((string)WxPayConfig::$NOTIFY_URL);//异步通知url
		}

		$inputObj->SetAppid((string)WxPayConfig::$APPID);//公众账号ID
		$inputObj->SetMch_id((string)WxPayConfig::$MCHID);//商户号
		$inputObj->SetSpbill_create_ip((string)$inputObj->GetSpbill_create_ip());//终端ip
		$inputObj->SetNonce_str((string)self::getNonceStr());//随机字符串

		//签名
		$inputObj->SetSign();
		$xml = $inputObj->ToXml();
		//file_put_contents(dirname(__FILE__).'/logwechat.txt',$xml,FILE_APPEND);
