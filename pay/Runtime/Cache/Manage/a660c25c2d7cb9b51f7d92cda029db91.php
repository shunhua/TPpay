                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <form class="layui-form layui-form-pane form-inline" action="" method="get">
                                ...
                                <div class="form-group">
                                    <input class="form-control" name="username" placeholder="收款人" value="<?php echo ($_REQUEST['username']); ?>" size="12" type="text">
                                </div>
                                <div class="form-group">
                                    <input class="form-control" name="ka" placeholder="账户" value="<?php echo ($_REQUEST['ka']); ?>" size="12" type="text">
                                </div>
                                <div class="form-group">
                                    <select name="ifcheck" class="layui-btn-small ajax-action form-control"  >
                                            <option value="">所有状态</option>
                                            <option value="0" <?php if($_REQUEST['ifcheck']== '0'): ?>selected="selected"<?php endif; ?>>未通过</option>
                                            <option value="1" <?php if($_REQUEST['ifcheck']== '1'): ?>selected="selected"<?php endif; ?>>通过</option>
                                    </select>
                                </div>
                        ...
                        <div class="set set0 table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <td colspan="10">
                                            <button class="btn btn-danger anniu delbtn" type="button" data-url="<?php echo U('Ka/delete');?>">删除</button>
                                            <button class="btn btn-success anniu addbtn" type="button" data-url="<?php echo U('Ka/add');?>">添加</button>
                                ...
                                <tbody>
                                    <?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$n): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($n["id"]); ?>" >
                                        ...
                                        <td align="center"><a href="<?php echo U('Ka/edit',array('id'=>$n['id'],'p'=>$_GET['p']));?>"><?php echo ($n["userid"]); ?></a></td>
                                        ...
                                        <td align="center">
                                            <a href="<?php echo U('Ka/edit',array('id'=>$n['id'],'p'=>$_GET['p']));?>" class="layui-btn layui-btn-normal layui-btn-mini">编辑</a>
                                            <a href="javascript:;" data-id="<?php echo ($n["id"]); ?>" data-url="<?php echo U('Ka/delete',array('id'=>$n['id'],'p'=>$_GET['p']));?>" class="layui-btn layui-btn-danger layui-btn-mini ajax-delete">删除</a>
                                        </td>
